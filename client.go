...
func (c *Client) Receive() {
	for msg := range c.Peer.Receive() {
		...
		case *Goodbye:
			log.Infof("client received Goodbye message")
			break
...
// 	} else {
// 		c.lock.RUnlock()
// 		log.WithField("registration_id": msg.Registration).Error("no handler registered for registration")
// 		if err := c.Send(&Error{
// 			Type:    INVOCATION,
// 			Request: msg.Request,
// 			Details: make(map[string]interface{}),
// 			Error:   URI(fmt.Sprintf("no handler for registration: %v", msg.Registration)),
// 		}); err != nil {
// 			log.WithField("error": err).Error("error sending message")
// 		}
// 		sync <- struct{}{}
// 	}
// 	<-sync
